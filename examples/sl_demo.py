...
def generate_sl(batch=1000,
    ...
    a,b,c,d: parameters of the Lotka-Volterra system
    ...
    original = sp_odeint(lv.sl, y0, orig_ts,
                         args=(alpha,beta,omega,I))
...
if __name__ == '__main__':
    ...
    # generate toy LV data
    orig_trajs, samp_trajs, orig_ts, samp_ts, sample_plot_range = generate_sl(
        batch=batch,
        noise_std=noise_std
    )
    ...
    try:
        ...
        for itr in range(1, args.niters + 1):
            ...
            if args.viz and itr % 10 == 0 or itr == 1:
                ...
                ax_traj.set_title('Traiectorii')
                ax_traj.set_xlabel('t')
                ax_traj.set_ylabel('concentrația (albastru), activitatea chimică (verde')
                ...
                plt.draw()
                plt.savefig('sl_png/{:03d}'.format(itr))
                plt.pause(0.001)
    ...
    if args.viz:
        ...
        plt.ylabel('Loss')
        plt.savefig('sl_train_loss.png')

    with open("sl_loss.txt", "wb") as fp:   # Pickling
